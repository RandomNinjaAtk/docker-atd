...
MusicBrainzMirrorUrl=https://musicbrainz.org
...
LidarrList () {
	...
	if [[ "$LidarrList" == "missing" || "$LidarrList" == "both" ]]; then
	...
	if [[ "$LidarrList" == "cutoff" || "$LidarrList" == "both" ]]; then
	...
	missinglistalbumids=($(cat "/scripts/lidarr-monitored-list.json" | jq -r '.records | sort_by(.artist.artistMetadataId) | .[].id'))
	missinglisttotal=$(cat "/scripts/lidarr-monitored-list.json" | jq -r '.records | sort_by(.artist.artistMetadataId) | .[].id' | wc -l)
...
WantedMode () {
	...
	for id in ${!missinglistalbumids[@]}; do
		...
		releases=$(curl -s -A "$agent" "${MusicBrainzMirrorUrl}/ws/2/release?release-group=$albumreleasegroupmbzid&inc=url-rels&fmt=json")
		albumreleaseid=($(echo "${releases}"| jq -r '.releases[] | select(.relations[].url.resource | contains("tidal")) | .id'))
		sleep $MusicBrainzRateLimit
		...
		logheader="$currentmissingprocess of $missinglisttotal :: $albumartistname :: $albumreleaseyear :: $lidarralbumtype :: $albumtitle"
		filelogheader="$albumartistname :: $albumreleaseyear :: $lidarralbumtype :: $albumtitle"
		echo $filelogheader
		artistdata=$(echo "${lidarrdata}" | jq -r ".[] | select(.foreignArtistId==\"${albumartistmbzid}\")")
		tidalurl=""
		tidalartistid=""
		tidalurl=$(echo "${artistdata}" | jq -r ".links | .[] | select(.name==\"tidal\") | .url")
		tidalartistid="$(echo "$tidalurl" | grep -o '[[:digit:]]*' | head -n 1)"
		...
		if [ "$tidalartistid" != "$Temptidalartistid" ]; then
			albums_data=$(curl -s "https://api.tidal.com/v1/artists/${tidalartistid}/albums?countryCode=$CountryCode&offset=0&limit=50&filter=ALL" -H "x-tidal-token: CzET4vdadNUFQ5JU")
			...
			if [ ! $albums_total == "null" ]; then
				if [ $albums_total -le 50 ]; then
					...
					log "$setlog $DL_TYPE :: FINDING ALBUMS"
						...
						for ((i=0;i<=$offsetcount;i++)); do
							if [ ! -f "release-page-$i.json" ]; then
								...
								curl -s "https://api.tidal.com/v1/artists/${tidalartistid}/albums?countryCode=$CountryCode&offset=$offset&limit=50&filter=ALL" -H "x-tidal-token: CzET4vdadNUFQ5JU" -o "/config/temp/${tidalartistid}-releases-page-$i.json"
								sleep 0.1
							fi
						done

						ArtistAlbums=$(jq -s '.' /config/temp/${tidalartistid}-releases-page-*.json)

						if [ -d "/config/temp" ]; then
							rm /config/temp/${tidalartistid}-releases-page-*.json
							sleep 0.1
							rm -rf "/config/temp"
						fi
					...
					#echo "$ArtistAlbums" > tempartist.json
					
				fi
				#echo "albums: $albums_total"
				albumcount=$(echo "$album_ids" | wc -l)
				echo "ablums found $albumcount"
				album_ids=($(echo "$album_ids"))
				
			fi
			Temptidalartistid=$tidalartistid
		fi

		LidarrAlbumReleaseIds=($(echo "$lidarralbumdata" | jq -r '.[].releases[].id'))
		for id in ${!LidarrAlbumReleaseIds[@]}; do
			currentprocess=$(( $id + 1 ))
			LidarrAlbumReleaseId="${LidarrAlbumReleaseIds[$id]}"
			...
			selected_album_ids=$(echo "$ArtistAlbums" | jq -r ".[].items | sort_by(.numberOfTracks) | sort_by(.explicit and .numberOfTracks) | reverse |.[] | select(.numberOfTracks==$LidarrAlbumReleaseTrackCount) | .id")
			selected_album_ids_count="$(echo "$selected_album_ids" | wc -l)"
			selected_album_ids=($(echo "$selected_album_ids"))
			if cat /config/logs/no_match.log | grep "^${LidarrAlbumReleaseId}$" | read; then
				echo "$currentprocess :: $LidarrAlbumReleaseId :: $LidarrAlbumReleaseTitle :: $LidarrAlbumReleaseTrackCount :: Previously not found, skipping..." 
				continue
			fi

			for id in "${!selected_album_ids[@]}"; do
				album_id=${selected_album_ids[$id]}
				TidalAlbumTitle="$(echo "$albums_data" | jq -r ".items[] | select(.id==$album_id) | .title")"
				diff=$(levenshtein "${LidarrAlbumReleaseTitle,,}" "${TidalAlbumTitle,,}")
				if [ "$diff" -le "1" ]; then
					log "$logheader :: ${LidarrAlbumReleaseTitle,,} vs ${TidalAlbumTitle,,} :: Distance = $diff :: $album_id :: MATCH"
					TidalAlbumMatchID="$album_id"
					break
				else
					log "$logheader :: ${LidarrAlbumReleaseTitle,,} vs ${TidalAlbumTitle,,} :: Distance = $diff :: $album_id :: ERROR :: NO MATCH FOUND"
					TidalAlbumMatchID=""
					TidalAlbumTitle=""
					continue
				fi
			done
			if [ -z $TidalAlbumMatchID ]; then
				echo "$LidarrAlbumReleaseId" >> /config/logs/no_match.log
				continue
			fi
			TidalAlbumTitleClean="$(echo "$TidalAlbumTitle" | sed -e "s%[^[:alpha:][:digit:]._()' -]% %g" -e "s/  */ /g")"
			artistclean="$(echo "$albumartistname" | sed -e "s%[^[:alpha:][:digit:]._()' -]% %g" -e "s/  */ /g")"
			if [ ! -d "$DownloadLocation/for_import/$artistclean - $TidalAlbumTitleClean ($albumreleaseyear)-WEB-$lidarralbumtype-streamrip" ]; then
				if [ "$StartClientSelfTest" = "TEST" ]; then
					ClientSelfTest
				fi
				ClientDownload "--max-quality $DownloadClientQuality https://tidal.com/browse/album/$TidalAlbumMatchID"

				albumbimportfolder="$DownloadLocation/for_import/$artistclean - $TidalAlbumTitleClean ($albumreleaseyear)-WEB-$lidarralbumtype-streamrip"
				mkdir -p "$DownloadLocation/for_import/$artistclean - $TidalAlbumTitleClean ($albumreleaseyear)-WEB-$lidarralbumtype-streamrip"
				find $DownloadLocation/temp -type f -exec mv {} "$DownloadLocation/for_import/$artistclean - $TidalAlbumTitleClean ($albumreleaseyear)-WEB-$lidarralbumtype-streamrip"/ \;
				...
				log "$logheader :: LIDARR IMPORT NOTIFICATION SENT! :: $albumbimportfolder"
			fi
