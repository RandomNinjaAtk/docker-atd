...
Configuration () {
	...
	if [ -d "/downloads-atd" ]; then
	    ...
	    log "ERROR: Music Video Library Location Not Found! (/downloads-atd)"
		log "ERROR: To correct error, please add a \"$LIBRARY\" volume"
		error=1
	fi
...
SelfTest () {
	...
	if find $LIBRARY/Album -type f -iname "*.m4a" | read; then
		log "SELF TEST :: SUCCESS"
		if [ -d $LIBRARY/Album ]; then
			rm -rf $LIBRARY/Album
		fi
	else
		if [ -d $LIBRARY/Album ]; then
...
AlbumProcess () {
	...
	if [ -d "$LIBRARY/music" ]; then
		if find "$LIBRARY/music" -type d -iname "* ($album_id)" | read; then
	...
	if [ "$album_artist_id" -ne "$artist_id" ]; then
		log "$albumlog ERROR :: ARTIST :: $album_artist_name ($album_artist_id) :: Not Wanted :: adding to artist list for future download..."
		if [ ! -f "/config/list/${album_artist_id}-complete" ]; then
			touch "/config/list/$album_artist_id-complete"
		fi	
		return
	fi
	if [ -d "$LIBRARY/music/$album_folder_name" ]; then
		log "$albumlog Already downloaded, skipping..."
		return
	fi
		
	if [ -d "$LIBRARY/music" ]; then
		if find "$LIBRARY/music" -type d -iname "$album_artist_name_clean ($album_artist_id) - $album_type - $album_release_year - $album_title_clean${album_version_clean} ([[[:digit:]][[:digit:]]*[[:digit:]][[:digit:]])" | read; then
			log "$albumlog Already downloaded, skipping..."
			return
		fi
	fi
	...
	for id in ${!track_ids[@]}; do
		...
		video_match_found=false
		if [ ! -f "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)/$video_file_name" ]; then
			video_titles="$(echo "$videos_data" | jq -r ".items[] | .title")"
			if echo "$video_titles" | grep "$track_title" | read; then
				matched_video_ids=($(echo "$videos_data" | jq -r ".items[] | .id"))
				for id in ${!matched_video_ids[@]}; do
					subprocess=$(( $id + 1 ))
					matched_video_id=${matched_video_ids[$id]}
					matched_video_id_data=$(echo "$videos_data" | jq -r ".items[] | select(.id==$matched_video_id)")
					matched_video_title="$(echo "$matched_video_id_data" | jq -r ".title")"
					matched_video_version="$(echo "$matched_video_id_data" | jq -r ".version")"
					video_image_id=$(echo "$matched_video_id_data" | jq -r ".imageId")
					video_image_id_fix=$(echo "$video_image_id" | sed "s/-/\//g")
					video_thumb=https://resources.tidal.com/images/$video_image_id_fix/750x500.jpg
					video_explicit=$(echo "$matched_video_id_data" | jq -r ".explicit")
					video_track_number=$(echo "$matched_video_id_data" | jq -r ".trackNumber")
					if [ "$video_image_id" == "null" ]; then
						video_thumb="null"
					fi
					if [ "$video_explicit" = "false" ]; then
						video_explicit="0"
					else
						video_explicit="1"
					fi
					
					if [ "$track_title" = "$matched_video_title" ]; then
						# log "TRACK TITLE MATCH FOUND!!!"
						if [ "$matched_video_version" = "$track_version_video" ]; then
							#log "TRACK VERSION MATCH FOUND!!!"
							if [ "$matched_video_version" == "null" ]; then
								matched_video_version=""
							else
								matched_video_version=" ($matched_video_version)"
							fi
							# echo $matched_video_title
							# echo $track_version_video
							# echo $matched_video_version
							video_match_found=true
							break
						else
							video_match_found=false
							continue
			...
			if [ "$video_match_found" = "true" ]; then
				log "$albumlog $track_id_number OF $track_ids_count :: VIDEO MATCH :: FOUND"
			else
				log "$albumlog $track_id_number OF $track_ids_count :: VIDEO MATCH :: NOT FOUND"
			fi
		else
			log "$albumlog $track_id_number OF $track_ids_count :: VIDEO MATCH :: ALREADY DOWNLOADED!"
		fi
		...
		deezer_track_bpm="null"
		deezer_track_data=$(curl -s https://api.deezer.com/2.0/track/isrc:${track_isrc})
		deezer_track_id=$(echo "$deezer_track_data" | jq -r ".id")
		
		if [ "$deezer_track_id" = "null" ]; then
			deezer_track_lyrics_text=""
			deezer_track_bpm="0"
			deezer_track_album_genres=""
			log "$albumlog $track_id_number OF $track_ids_count :: Deezer Track Data Not Found..."
		else
			log "$albumlog $track_id_number OF $track_ids_count :: Deezer Track Data Found :: ID :: $deezer_track_id"
			deezer_track_bpm=$(echo "$deezer_track_data" | jq -r ".bpm")
			deezer_track_bpm=$(echo "$deezer_track_bpm" | cut -d. -f1)
			deezer_track_lyrics_data=$(curl -s curl "https://www.deezer.com/ajax/gw-light.php?method=song.getLyrics&api_version=1.0&api_token=_FqFEK7%7E3iw6l4W0gS8nh5GUkAmxArZq&cid=331078903&sng_id=$deezer_track_id" -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:87.0) Gecko/20100101 Firefox/87.0' -H 'Accept: */*' -H 'Accept-Language: en-US,en;q=0.5' --compressed -H "Referer: https://www.deezer.com/us/track/$deezer_track_id" -H 'x-deezer-user: 0' -H 'Content-Type: text/plain;charset=UTF-8' -H 'Origin: https://www.deezer.com' -H 'DNT: 1' -H 'Connection: keep-alive' -H 'Cookie: sid=fr0e4674ea116aed0d1f647f343e330868396834; ak_bmsc=218174B2BCCE8A4213C1EE63F1758BB41737398F336D0000063B7860061EF655~; bm_sz=4808D69F35650DFB134B92BC07AA9EBA~YAAQjzk3F1hYh8h4AQAAqI+m1QuMdi+yS4qEZfhRZUNWaZLCID7mExRxhTHN6YWtp5zLLOex208RiCDH5EJbieml50QAX2DXD+zdQKxUEZfSprO1DGgvdOSNfG33+H5A93t8sMusOeDurtvMjXtDH87dlsCsRwcvXixjnGnImOK0nwR6KCAYNNkjOs5QFWY=; _abck=3FEFB83FD9F39D20609E3017D25B170F~0~~-1~||-1||~-1; bm_sv=3BF0885AA0C4E2717D4482A1C5DD080F~ifvDnLVFH6w0PmInzD6MDSuAwSaMH6moZFdUWGnEyuDDKfrlf3q9XlZR1M9cKuO0eIc6yjSUyeiWDFsrl82qdEBY1k5PnTUFmHF9a14RAigEgiM/XmIMxONHKcGOAvUIcsMaDqtX2KQ2NkXPxrAmnFzZQGFCC3gjF40zu7vdryc=; dzr_uniq_id=dzr_uniq_id_frcd45a6c5aeb2e1ce94617276ed648af4d4212f; bm_mi=736C48C321C9EB5CF209CEBB7DD4D6A7~; euconsent-v2=CPEslA5PEslA5A7ACAENBVCsAP_AAH_AAAqIHsNf_X__b39j-_59__t0eY1f9_7_v-0zjhfdt-8N2f_X_L8X42M7vF36pq4KuR4Eu3LBIQNlHOHUTUmw6okVrTPsak2Mr7NKJ7LEinMbe2dYGHtfn91TuZKYr_78_9fz__-__v___9f3r-3_3__p9X---_e_V399xLv9__wPVAJMNS-ACzEscCSaNKoUQIQrCQ6AUAFFCMLRNYQMrgp2VwEQoIGACE1ARgRAgwBRiQCAAQCAICIgJABwQAIAiAQAAgBUgIQAESAAKACwMAgAFANCxAiACEAQgyICo5DAgIkWCgnsqAEAstjDCEAosAIBQ9QAICJAAASBkAAA.flgAAAAAAAAA; noIabVendors=[1]; consentStatistics=1; consentMarketing=1; ry_ry-d33z3r2s_realytics=eyJpZCI6InJ5X0I1NTJGMEUwLTM4MUYtNDM0RC1BOTUzLThCMzNDMjQ1NTYyQiIsImNpZCI6bnVsbCwiZXhwIjoxNjUwMDI4MTcxODIxLCJjcyI6bnVsbH0%3D; ry_ry-d33z3r2s_so_realytics=eyJpZCI6InJ5X0I1NTJGMEUwLTM4MUYtNDM0RC1BOTUzLThCMzNDMjQ1NTYyQiIsImNpZCI6bnVsbCwib3JpZ2luIjp0cnVlLCJyZWYiOm51bGwsImNvbnQiOm51bGwsIm5zIjpmYWxzZX0%3D; _scid=02ce9335-fbe7-4e6e-8dd8-0ffd2423ddac; tg=d41d8cd98f00b204e9800998ecf8427e; _tgpc=2291773b-b5aa-5f22-a200-81a8b50e9fc5; _tgrsid=2fab5a08-0855-5845-ab8f-3aaf2c4f90bf; _tglksd=eyJzIjoiMmZhYjVhMDgtMDg1NS01ODQ1LWFiOGYtM2FhZjJjNGY5MGJmIiwic3QiOjE2MTg0OTIxNzE5MzR9; _tgsc=2fab5a08-0855-5845-ab8f-3aaf2c4f90bf:-1; _tgsource=(direct); _gcl_au=1.1.776482461.1618492173; _tgtim=2fab5a08-0855-5845-ab8f-3aaf2c4f90bf:1618492175110:-1' -H 'TE: Trailers' --data-raw '{"consent_string":"CPEslA5PEslA5A7ACAENBVCsAP_AAH_AAAqIHsNf_X__b39j-_59__t0eY1f9_7_v-0zjhfdt-8N2f_X_L8X42M7vF36pq4KuR4Eu3LBIQNlHOHUTUmw6okVrTPsak2Mr7NKJ7LEinMbe2dYGHtfn91TuZKYr_78_9fz__-__v___9f3r-3_3__p9X---_e_V399xLv9__wPVAJMNS-ACzEscCSaNKoUQIQrCQ6AUAFFCMLRNYQMrgp2VwEQoIGACE1ARgRAgwBRiQCAAQCAICIgJABwQAIAiAQAAgBUgIQAESAAKACwMAgAFANCxAiACEAQgyICo5DAgIkWCgnsqAEAstjDCEAosAIBQ9QAICJAAASBkAAA.flgAAAAAAAAA","listening_context":{},"track_list":[]}')
			sleep 0.1
			#echo "$deezer_track_lyrics_data"
			deezer_track_lyrics_text="$(echo "$deezer_track_lyrics_data" | jq -r ".results.LYRICS_TEXT")"
		
			if [ "$deezer_track_lyrics_text" = "null" ]; then
				deezer_track_lyrics_text=""
			fi
			
			if [ "$deezer_track_bpm" = "null" ]; then
				deezer_track_bpm="0"
			fi
			deezer_track_album_id=$(echo "$deezer_track_data" | jq -r ".album.id")
			if [ "$deezer_track_album_id" != "null" ]; then
				deezer_track_album_data=$(curl -s https://api.deezer.com/album/${deezer_track_album_id})
				deezer_track_album_genres=$(echo "$deezer_track_album_data" | jq -r ".genres.data[].name" | head -n 1)
			fi
		fi
		...
		#echo $track_version
		#echo $track_explicit
		#echo $track_volume_number
		#echo $track_track_number
		#echo $track_artist_names
		#echo $track_isrc
		#echo "track artist id: $track_artist_id :: $track_artist_name"
		#echo "Procuder: $track_producer_names"
		#echo "Composer: $track_composer_names"
		# echo "Lyricist: $track_lyricist_names"
		# echo "Mixer: $track_mixer_names"
		# echo "Studio: $track_studio_names"
		...
		#find $LIBRARY/Album -type f -iname "*.m4a" -exec qtfaststart "{}" \; &>/dev/null
		file="$(find $LIBRARY/Album -type f -iname "*.m4a")"
		filelyric="$(find $LIBRARY/Album -type f -iname "*.lrc")"
		...
		mv "$file" "$LIBRARY/Album/$track_file_name"
		if [ -f "$filelyric" ]; then
			mv "$filelyric" "$LIBRARY/Album/$track_file_lyric_name"
		fi
		file="$LIBRARY/Album/$track_file_name"
		...
		python3 /config/tag-m.py \
			...
			--songartwork "$LIBRARY/Album/cover.jpg"
		...
		if [ -d "$LIBRARY/Album" ]; then
            find $LIBRARY/Album -type f -exec mv "{}" "$LIBRARY/temp/" \;
			rm -rf "$LIBRARY/Album"
        fi
	...
	download_count=$(find $LIBRARY/temp -type f -iname "*.m4a" | wc -l)
	...
	#find $LIBRARY/Album -type f -iname "*.m4a" -exec mv "{}" $LIBRARY/Album/ \; &>/dev/null
	...
	if [ ! -d "$LIBRARY/music/$album_folder_name" ]; then
		mkdir -p "$LIBRARY/music/$album_folder_name"
	fi
	if [ -d "$LIBRARY/temp" ]; then
		mv $LIBRARY/temp/* "$LIBRARY/music/$album_folder_name"/
	fi
	if [ -d "$LIBRARY/temp" ]; then
		rm -rf "$LIBRARY/temp"
	fi
	
	nfo="$LIBRARY/music/$album_artist_name_clean ($album_artist_id)/artist.nfo"
	if [ ! -f "$nfo" ]; then
		...
		if [ "$artist_picture_id" == "null" ]; then
			...
			curl -s "$thumb" -o "$LIBRARY/music/$album_artist_name_clean ($album_artist_id)/poster.jpg"
	...
	nfo="$LIBRARY/music/$album_folder_name/album.nfo"
	if [ -d "$LIBRARY/music/$album_folder_name" ]; then
		...
		if [ ! -f "$nfo" ]; then
			...
			echo "		<musicBrainzArtistID/>" >> "$nfo"
			echo "	</albumArtistCredits>" >> "$nfo"
			if [ -f "$LIBRARY/music/$album_folder_name/cover.jpg" ]; then
...
AddMissingArtists () {
	if [ -d $LIBRARY/music ]; then
		completeartistlist=($(find $LIBRARY/music -maxdepth 1 -mindepth 1 | grep -o '(*[[:digit:]]*)' | sed 's/(//g;s/)//g' | sort -u))
...
VideoDownload () {
	...
	if [ -d "$LIBRARY/Video" ]; then
		rm -rf "$LIBRARY/Video"
	fi
	tidal-dl -l "https://tidal.com/browse/video/$matched_video_id"
	find "$LIBRARY/Video" -type f -iname "*.mp4" -print0 | while IFS= read -r -d '' video; do
		count=$(($count+1))
		file="${video}"
		...
		if [ "$video_thumb" = "null" ]; then
			ffmpeg -y \
				-i "$file" \
				-vframes 1 -an -s 640x360 -ss 30 \
				"$LIBRARY/Video/thumb.jpg" &> /dev/null
			curl -s "$video_thumb" -o "$LIBRARY/Video/thumb.jpg"
		else
			curl -s "$video_thumb" -o "$LIBRARY/Video/thumb.jpg"
		fi

		mv "$file" "$LIBRARY/Video/temp.mp4"
		cp "$LIBRARY/Video/thumb.jpg" "$LIBRARY/Video/cover.jpg"
		log "========================START FFMPEG========================"
		ffmpeg -y \
			-i "$LIBRARY/Video/temp.mp4" \
			-map 0:v \
			-map 0:a \
			-c copy \
		...
		python3 /config/tag.py \
			...
			--songartist "$track_artist_names" \
			--songartistalbum "$album_artist_name" \
			--songtracknumber "$video_track_number" \
			--songlyricrating "$video_explicit" \
			--songgenre "$deezer_track_album_genres" \
			--songdate "$album_release_year" \
			--quality "$videoquality" \
			--mbrainzalbumartistid "$musicbrainz_main_artist_id" \
			--songartwork "$LIBRARY/Video/cover.jpg"
		rm "$LIBRARY/Video/temp.mp4"		
		
		if [ ! -d "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)" ]; then
			mkdir -p "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)"
		fi
	
		mv "$file" "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)/$video_file_name"
		mv "$LIBRARY/Video/cover.jpg" "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)/${video_file_name%.mp4}.jpg"
		cp "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)/$video_file_name" "$LIBRARY/temp/$track_video_file_name"
		cp "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)/${video_file_name%.mp4}.jpg" "$LIBRARY/temp/${track_video_file_name%.mp4}.jpg"
		if [ ! -f "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)/artist.nfo" ]; then
			cp "$LIBRARY/music/$album_artist_name_clean ($album_artist_id)/artist.nfo" "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)/artist.nfo"
		fi
		if [ ! -f "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)/poster.jpg" ]; then
			cp "$LIBRARY/music/$album_artist_name_clean ($album_artist_id)/poster.jpg" "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)/poster.jpg"
		fi
		#cp "$LIBRARY/Video/cover.jpg" "/$destination/$clean_main_artists_name - $clean_title${clean_version} ($videoid).jpg"
	done
	
	nfo="$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)/${video_file_name%.mp4}.nfo"
	if [ -f "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)/$video_file_name" ]; then
	...
	if [ ! -f "$nfo" ]; then
		echo "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\" ?>" >> "$nfo"
		echo "<musicvideo>" >> "$nfo"
		echo "	<title>$track_title${track_version}</title>" >> "$nfo"
		echo "	<userrating/>" >> "$nfo"
		if [ $video_track_number == 0 ]; then
			echo "	<track/>" >> "$nfo"
		else
			echo "	<track>$video_track_number</track>" >> "$nfo"
		fi
		echo "	<album>Music Videos</album>" >> "$nfo"
		echo "	<plot/>" >> "$nfo"
		echo "	<director/>" >> "$nfo"
		echo "	<premiered>$album_release_date</premiered>" >> "$nfo"
		echo "	<year>$album_release_year</year>" >> "$nfo"
		echo "	<studio/>" >> "$nfo"
		echo "	<genre>$deezer_track_album_genres</genre>" >> "$nfo"
		OLDIFS="$IFS"
		IFS=$'\n'
		nfo_track_artist_name=($(echo "$track_data" | jq -r ".artists[] | .name"))
		IFS="$OLDIFS"
		for name in ${!nfo_track_artist_name[@]}; do
			artist_name="${nfo_track_artist_name[$name]}"
			echo "	<artist>$artist_name</artist>" >> "$nfo"
		done
		if [ -f "$LIBRARY/videos/$album_artist_name_clean ($album_artist_id)/${video_file_name%.mp4}.jpg" ]; then
			echo "	<thumb>${video_file_name%.mp4}.jpg</thumb>" >> "$nfo"
		else
			echo "	<thumb/>" >> "$nfo"
		fi
		echo "</musicvideo>" >> "$nfo"
...
for id in ${!list[@]}; do
	artistnumber=$(( $id + 1 ))
	artist_id="${list[$id]}"
	artisttotal="$listcount"
	logheader="$artistnumber of $listcount"
	SelfTest
	ProcessArtist
done
